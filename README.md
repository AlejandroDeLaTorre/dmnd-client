...
Choose one of the two options below depending on your preference.
...
SHA256 checksums are provided on the releases page for each binary. Verify before running.
...
```bash
chmod +x dmnd-client-macos
TOKEN=<DMND-token> ./dmnd-client-macos -l info -d <avg-hashrate>T --tp-address="127.0.0.1:<port>"
```

> **macOS note:** If macOS blocks the binary with a security warning, run the following once before
> executing:
> ```bash
> xattr -d com.apple.quarantine dmnd-client-macos
> ```
...
```cmd
set TOKEN=<DMND-token>
dmnd-client-windows.exe -l info -d <avg-hashrate>T --tp-address="127.0.0.1:<port>"
```
...
TOKEN=abc123 ./dmnd-client-macos -l info -d 200T --tp-address="127.0.0.1:8336"
set TOKEN=abc123 && dmnd-client-windows.exe -l info -d 200T --tp-address="127.0.0.1:8336"
```

### Option B — Build from Source

Requires [Rust](https://rustup.rs/) to be installed. Clone the repository and run:

```bash
git clone https://github.com/dmnd-pool/dmnd-client.git
cd dmnd-client
...
**Parameters (applies to both options):**
